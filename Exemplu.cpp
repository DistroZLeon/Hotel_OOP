...
class hotel{
        ...
        friend istream& operator>>(istream& in,hotel &h){
            in>>h.nr_apartamente>>h.nr_camere>>h.nr_sali_indiv>>h.nr_sali_scena;
            int t=0;
...
void data_base::asezare(int idul){
    int zi_minima=1,gasit=0,accept=1,nr_sali_necesare=0,apartam[30],came[30],sala_indivi[10],salascena,nr_apart=0,nr_cam=0,nr_individ=0;
    if(tip_sala==0&&nr_participanti<hotelul.get_capacitate_minima(hotelul.get_nr_apartamente()+hotelul.get_nr_camere()))accept=0;
